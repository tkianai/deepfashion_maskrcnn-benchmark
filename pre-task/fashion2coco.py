...
def parse_args():
    parser = argparse.ArgumentParser(description="Change DeepFashion2 dataset to coco style")
    parser.add_argument('--root', default='datasets/fashion', help="The original directory structure")
...
def _transform(root):
    files = os.listdir(os.path.join(root, 'image'))
    files = [e for e in files if e.endswith('jpg')]
    ...
    for num in range(1, num_images+1):
        json_name = os.path.join(root, 'annos', str(num).zfill(6)+'.json')
        image_name = os.path.join(root, 'image', str(num).zfill(6)+'.jpg')
    ...
    json_name = os.path.join(root, 'deepfashion2_coco.json')
    with open(json_name, 'w') as f:
        json.dump(dataset, f)

def main(args):

    _transform(os.path.join(args.root, 'train'))
    _transform(os.path.join(args.root, 'validation'))
